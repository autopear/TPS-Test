...
int main(int argc, char *argv[]) {
  if (argc != 11) {
    ...
    std::cout << "    -  file-ratio: \"m,n\". Random number of files between m "
                 "and n. "
              << std::endl;
    std::cout
        << "                   m and n can be float (ratio) or int (numbers)"
        << std::endl;
    std::cout << "    -  size-ratio: \"m,n\". Scan size ratio of each file "
                 "between m and n. "
              << std::endl;
    std::cout
        << "                   m and n can be float (ratio) or int (bytes)"
        << std::endl;
    std::cout << "    -    seq-file: If true, order files to scan sequentially "
                 "by their names."
              << std::endl;
    std::cout << "                   {true, t, yes, y, 1, false, f, no, n, 0}"
              << std::endl;
  ...
  bool buffered = true;
  int num_threads = 1;
  tps::Bounds ex_bounds;
  tps::Bounds in_bounds;
  bool seq_file = true;
  bool seq_scan = true;
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg.first.compare("dir") == 0)
      ...
      if (min.find_first_of(".") == std::string::npos &&
        ...
        ex_bounds.set_sizes(tps::to_size_t(min), tps::to_size_t(max));
      } else {
        ex_bounds.set_ratios(std::stod(min), std::stod(max));
      }
      ...
      if (min.find_first_of(".") == std::string::npos &&
          max.find_first_of(".") == std::string::npos) {
        in_bounds.set_sizes(tps::size_in_bytes(min), tps::size_in_bytes(max));
      } else {
        in_bounds.set_ratios(std::stod(min), std::stod(max));
      }
  ...
  tps::FileScan fs(dir_path, record_size, max_time, buffered, num_threads,
                   ex_bounds, in_bounds, seq_file, seq_scan, full_middle);
  fs.print_arguments();
  fs.start_read();
